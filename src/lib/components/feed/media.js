export function ShowMedia(lists, screenWidth) {
    ...
    if (
        ...
        if (count === 1) {
            // One image
            example.push(`<div class="img-alone"><img style="width:100%;object-fit: cover;" src="${media[0]}" /></div>`);
        } else if (count === 2) {
            // Two images side-by-side
            example.push(`<div style="display:flex;" class="img-together">
                <img style="width:50%;object-fit: cover;" src="${media[0]}" />
                <img style="width:50%;object-fit: cover;" src="${media[1]}" />
            </div>`);
        } else if (count === 3) {
            // One big top image, two below
            example.push(`<div class="image-big"><img src="${media[0]}" /></div>`);
            ...
            example.push(`<div style="display:flex;" class="image-sizing">
                <img style="width:50%;object-fit: cover;" src="${media[2]}" />
                <img style="width:50%;object-fit: cover;" src="${media[3]}" />
            </div>`);
        } else if (count > 4) {
